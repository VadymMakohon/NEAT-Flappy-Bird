...
class Bird:
    ...
    def move(self):
        ...
        displacement = min(displacement, MAX_VELOCITY)
        if displacement < 0:
            displacement -= 2
        self.y += displacement
        self.tilt = self.MAX_ROTATION if displacement < 0 or self.y < self.height + 50 else self.tilt - self.ROT_VEL
        self.tilt = max(self.tilt, -90)

    def draw(self, win):
        self.img_count += 1
        self.img = bird_images[self.img_count // self.ANIMATION_TIME % len(bird_images)]
        if self.tilt <= -80:
